...
Before starting data exploration, a summary table is necessary to have a quick glimpse at the data.
...
```{r, figures-1b-cases, fig.show="hold", out.width="50%"}
#Fig 1b - same as 1c  but broken down by district
tar_read(fig_1b_1)
```

```{r, figures-1b-ar, fig.show="hold", out.width="50%"}
tar_read(fig_1b_2)
```

```{r, figures-1b-deaths, fig.show="hold", out.width="50%"}
tar_read(fig_1b_3)
```

```{r, figures-1b-cfr, fig.show="hold", out.width="50%"}
tar_read(fig_1b_4)
```
...
Even if cumulative data are the standard way to collect those information, additional variables have been calculated in the pre-processing to show the cumulative ones only in the previous 4 weeks, which provide a different picture.

Fig 1d - Three examples of different epi statuses situation w/colours 

In addition to that, the univariate distribution of the key epidemiological variables provide some additional information on its shape and variability across the database.
...
### Data exploration and visualisation - bidimensional trends
