...
-   Dataset will be analysed for temporal correlation from a surge in cases through a Distributed Lag Model, identifying the best lead time for the following step. Pooled estimates are obtained to verify the temporal correlation of that with a meta-analytical approach.
...
-   Two models are tested: the first uses the number of cases, the second uses the districts' attack rates as input variables. All of them are BYM models with the 3-level classification as a covariate.
...
Results and discussion follow the pipeline outlined above. Data integrity information is checked before the results are presented.

Fig 1a - gtsummary

### Data exploration and visualisation - Temporal trends

The four main variables used to track a cholera outbreak are the number of cases, the attack rate, deaths, and the Case Fatality Rate. To get a general picture, the absolute values (since the start of the outbreak) are visualised, both by district and cumulatively.
...
Different dynamics are shown across the 21 governorates analysed, with some of them (Abyan, Ad Dali, Shabwa, ...) showing a stop after the initial increase, while others that never managed to control the outbreaks (Dhamar, Sanaa, both rural and city, Raymah,...).

Another interesting phenomenon is the sudden spikes in the first derivative, as can be observed in Sanaa (rural), Hadramawt, or Shabwah, where the new cases are grouped in a short timeframe that is indicative of an immediate worsening of the situation.

```{r, figures-1b-ar}
tar_read(fig_1b_2)
```

The attack rate is a fundamental variable that drives the resources needed to control an outbreak. Since it's calculated on the new cases and the population, the attack rate follows a similar shape to the new cases, but it is the peak most indicative. Districts such as Al Mahwit and Amran (AR = 75 per thousand) are across the most affected governorates, while in Sadah (AR = 12) and Hadramawt (AR = 1.5), most of the people managed to avoid the infection. Other drivers may explain that the limited testing is due to volatile access conditions, but in general, the attack rate analysis shows different dynamics across the country.

```{r, figures-1b-deaths}
tar_read(fig_1b_3)
```

Deaths are a hard outcome that is usually easy to report at the Health Facility level compared to other indicators. Even in this case, there is significant variability across the regions, from only a few recorded deaths (Sadah, Shabwah) to hundreds of them (Amran, Ibb, Taiz,...). Since cholera does not usually cause permanent disabilities, death is the most important outcome to be monitored, but as for the absolute number of new cases, to understand the implication, a new metric is needed.
...
Case-Fatality Rate, apart from setting standards for the assistance provided (should be \<1%), is also a factor that shows the efficacy of the health system as a whole. The dataset shows the absolute CFR (from the start of the outbreak), so it's the only variable that is steadily decreasing. This, though, does not provide information on the moment-by-moment evolution of the epidemic.

In addition, it is notable that a spike was seen at the beginning of the outbreak in most of the regions. This is due to a combination of factors: at the beginning, confirmed cases are low and are usually the worst in terms of clinical conditions. Scaling up the health system capacity and risk communication activities are also effective ways to lower overall mortality (and fatality).
...
Even though cumulative data are the standard way to collect that information, additional variables have been calculated in the pre-processing to show the cumulative ones only for the previous four weeks. This provides a different picture, more representative of what happened in the previous month for that particular week.
...
All of the univariate distributions are left skewed, as usual for outbreaks (negative death values are related to reporting errors). Combined, they show a situation where, in most districts, the situation was managed in a way that avoided a humanitarian disaster. In fact, even in particular areas where there were situations of dire humanitarian needs, the concerted efforts of the health authorities and international partners managed to contain the uncontrolled spread of the diseases.
...
-   An orange area is an area with an active outbreak where the health system is responding effectively

-   A red area is an area where the health system resources are not enough to keep either the CFR under the 1% threshold (representing a situation where sick people access the proper care and the adequate number of cases is intercepted by healthcare staff), or an attack rate over 2 (x1000) signalling a high level of community transmission, or failure of the mitigation measures.

Combined, the status provides a glimpse of the situation in the country. This is a useful target variable to predict, given the direct questions it can answer for managing humanitarian operations.

### Data exploration and visualisation - Correlations

As a preliminary step, correlations with the key epidemiological variables are checked before checking additional modelling strategies.
...
A certain degree of correlation between monthly cases and deaths for the different weeks is expected. The linear model fitter on the bidimensional density plot. The correlation is more robust for many new cases, and the trend is roughly linear. Since the disease takes time to kill an infected person, this initial result calls for a lagged correlation model to verify if the link is stronger.
...
The same pattern is not so evident for the Case Fatality Rate, even if more cases are associated with a higher CFR in the same period. Even in this case, the preliminary model suggests a different approach incorporating lag considerations of a couple of weeks.
...
How the status variable was built forces a certain level of correlation among the variables. This is less evident for the first two variables (cases and deaths) since the Attack rate and CFR are used to determine them.

### Time series

There are several methods to control for long-term and seasonal patterns in time series studies. Given the limited number of years, no seasonality can be modelled. The long-term component is added to the model since the cases' growth slowly increased. It is expected to be non-linear by applying splines to model smooth temporal trends, initially with 5 degrees of freedom for each year of data.

```{r}
tar_read(epi_conf)
```

The initial model is a more sophisticated version of the plot shown above, clearly indicating that the epidemiological variables and the temporal components affect the outcome. This association is not very strong, as the residual deviance is similar to the Null one. Thus, it is difficult to determine the status from the original variables, but that is expected since the presented model does not consider any lag effect or geographical considerations.

In the second iteration, the association between the status and the four epidemiological variables (in the previous month) is evaluated, accounting for lagged associations up to four weeks. A bidimensional exposure-lag-response function is defined (cross-basis), with four different models that can be fit from it.
...
The effect at a particular cut-off and the overall cumulative association are shown. None of the charts contain any significant result for any lag of 1 week or more. Thus, modelling only from epidemiological variables is insufficient to predict the status accurately. In addition, the residual deviance from the models (not shown) is similar to the initial model, without any benefit in the residual variance from the additional lag considerations.

The last iteration is to verify the association at the single governorate level, and a dual-stage approach is used:
...
This does not consider the effect of geographical proximity, only the temporal association across the estimates.
...
Modelling ecological trends requires the definition of the proper target variable. In each week, the model will try to predict the status of that district in the following one (so with a one-week lag).

Three models are tested, with different epidemiological variables (either cases or attack rate, as they were the only ones in the previous section to show some cumulative association) and the status of the districts as covariates. A Poisson-gamma model with no spatial consideration is used to provide a reference.

#### Model 0 - no spatial component

The first model is a simple Poisson-gamma model with no spatial consideration. The variables are the expected number of cases and the status in the previous week, which are used to predict the status in the following week.

```{r}
tar_read(mod0) |> summary()
```

The results, despite showing a significant association for both variables, are non-indicative by themselves and should be considered a baseline.

#### Model 1 - Cases

The first BYM Model is similar to the former but has additional spatial considerations.
...
After properly initialising this model, we can provide a summary of the estimated coefficient of status. This shows a tendency towards a negative value of the Status variable, where higher statuses (equal to "worse situation") are inversely associated with a higher status in any particular region.
...
Cholera spatial dynamics remain elusive as we try to model them. This short report analysed some temporal and spatial patterns (but not the two together) as an exploratory moment before different techniques are tested.

The status classification doesn't fully capture the outbreak evolution, as the two analyses showed, but it is somewhat indicative of predictive power for the following week's evolution. The negative coefficient derived in both BYM models, compared with the close-to-zero one, shows that when spatial relationships are accounted for, there is an inverse relationship between a district's status and the ones of the others.
