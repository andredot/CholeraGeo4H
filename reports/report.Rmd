...
-   **Lower mortality** through Oral Rehydration Solution distribution in established points (ORPs) or outposts. This is critical, as the local health system may not have the capacity to develop the required number of ORPs.
...
According to the World Bank, Yemen is a lower-middle-income country with a more than 26 million population. However, due to ongoing conflicts, its population health indicators are some of the lowest in the world[@wb].
...
-   Risk Analysis and Early Warning systems in the health sector are radically different since forecasting models are of a different nature. Also, it is widespread in the Health sector to base clinical and public health decisions on risk assessment tools instead of relying on hard thresholds. This is done to enable contextual information, difficult to capture in triggers, to be part of the decision process [@who_risk]
...
plot(cp_cases, "slices", var=1200, ci="bars", pch=19, col=2, main="Alternative lag-response functions at Cases = 1200")
plot(cp_cases, "overall", ci="lines", col=2, lty=2,
      main="Overall cumulative association", lwd=2, xlab="New cases")

```

```{r,figures-3a-2, eval = FALSE, fig.show="hold", out.width="50%"}
...
```{r,figures-3a-3,eval = FALSE, fig.show="hold", out.width="50%"}
...
```{r,figures-3a-4,eval = FALSE, fig.show="hold", out.width="50%"}
